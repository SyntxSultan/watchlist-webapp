from flask import Blueprint, render_template, redirect, request,session, current_app, url_for, abort, flash
...
def register():
    ...
    if form.validate_on_submit():
        ...
        flash("User registered succesfully", category="succes")
        return redirect(url_for(".login"))

    return render_template("register.html", title="Watchlist - Register", form=form)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and pbkdf2_sha256.verify(form.password.data, user.password):
            ...
            return redirect(url_for(".index"))
        flash("Login credentials not correct!", category="danger")
    return render_template("login.html", title="Watclist - Login", form=form)
...
def add_movie():
    ...
    return render_template("new_movie.html", title="Watchlist - Add Movie", form=form)
